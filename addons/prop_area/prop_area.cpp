...
void PropArea::_register_methods()
{
	...
	register_property<PropArea, Ref<Mesh>>("mesh", &PropArea::set_mesh, &PropArea::get_mesh, nullptr);
	REGISTER_PROPERTY_HINT(PropArea, int64_t, collision_mask, -1, GODOT_PROPERTY_HINT_LAYERS_3D_PHYSICS, String());
	register_property<PropArea, int64_t>("layers", &PropArea::set_layers, &PropArea::get_layers, 1);
}
...
void PropArea::_init()
{
	...
	multi_mesh_instance = MultiMeshInstance::_new();
	multi_mesh_instance->set_cast_shadows_setting(GeometryInstance::SHADOW_CASTING_SETTING_OFF);
	multi_mesh_instance->set_multimesh(Ref<MultiMesh>(MultiMesh::_new()));
...
void PropArea::_ready()
{
	...
	curve2d = Ref<Curve2D>(Curve2D::_new());
...
void PropArea::refresh()
{
	if (!path)
		return;
	if (!in_tree) {
		Godot::print_warning("Attempting to refresh while not in tree.", __func__, __FILE__, __LINE__);
		return;
	}
	if (path->get_curve()->get_point_count() == 0)
		return;
	if (IN_EDITOR && has_meta("_edit_lock_"))
		return;
	...
	for (int64_t i = 0; i < number_of_instances; ++i) {
		...
		t.scale(base_scale);
		t.rotate(Vector3(0, 1, 0), DEG2RAD(base_rotation.y));
		t.rotate(Vector3(0, 0, 1), DEG2RAD(base_rotation.z));
		t.rotate(Vector3(1, 0, 0), DEG2RAD(base_rotation.x));
		t.translate(base_position);

		Vector2 offset;
		int j;
		for (j = 0; j < 1000; ++j) {
			offset = Vector2(rng->randf_range(curve_box.position.x, curve_box.position.x + curve_box.size.x), rng->randf_range(curve_box.position.y, curve_box.position.y + curve_box.size.y));
		...
		if (randomize_rotation) {
			t.rotate(Vector3(0, 1, 0), rng->randf_range(0, 2*M_PI));
		}
